...
void SetCodeSegment(SegmentDescriptor &desc, DescriptorType type, unsigned int descriptor_privilege_level, uint32_t base, uint32_t limit)
{
    ...
    desc.bits.default_operation_size = 0; // long_mode == 1 ならば 0 に設定する
...
{
    gdt[0].data = 0;
    SetCodeSegment(gdt[1], DescriptorType::kExecuteRead, 0, 0, 0xffff);
    SetDataSegment(gdt[2], DescriptorType::kReadWrite, 0, 0, 0xffff);
